#! /usr/bin/python
'''
This script creates a custom map file from a tab/vcf (CHROM,POS) file and
a known map.

It is very slow.
The loop [for l in range(0,len(maplist)): ...] needs to be replaced with
a better approach.
...
$ python make_custom_map.py -i input -m input.map -o new.map
...
parser.add_argument(
    '-m',
    '--input_map',
    help='name of the reference map file',
    type=str,
    required=True)
        ...
        if chr == mapChrPrev and chr == mapChr:
            if pos < mapPosPrev or pos == mapPosPrev:  # before reference SNPs
                ...
                if chr == mapChr and pos == mapPosPrev:
                    ...
                    output.write("%s\t%s_%s\t%s\t%s\n" % (chr, chr, pos,
                                                          mapMPrev, pos))
                    #print "unclassified", mapChrPrev, mapPosPrev, \
                    # chr, pos, mapChr, mapPos
        else:
            while (chr != mapChrPrev) or \
                (chr == mapChr and mapPosPrev < pos and pos > mapPos):
                #print "read new line", mapChrPrev, mapPosPrev,  \
                # chr, pos, mapChr, mapPos
                mapChrPrev = mapChr
                mapPosPrev = mapPos
                mapMPrev = mapM
                ...
                mapPos = int(mapwords[1])
                mapM = float(mapwords[3])
                #print "have read new line", mapChrPrev, mapPosPrev,  \
                # chr, pos, mapChr, mapPos
            if chr == mapChrPrev and chr == mapChr:
                if pos < mapPosPrev or pos == mapPosPrev: #before ref SNPs
                    #print "before or match", mapPosPrev, pos, mapPos
                    output.write("%s\t%s_%s\t%s\t%s\n" % (chr, chr, pos,
                                                          mapMPrev, pos))
                elif mapPosPrev < pos and pos < mapPos:
